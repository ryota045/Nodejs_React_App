...
const Day = styled.div`
  background-color: ${props => ( props.currentMonth ?
    `#eee` :
    `#ccc`
  )};
  text-align: center;
...
const DayComponent = ({ date, currentMonth,tournamentList, isModalVisible, onMouseOver, onMouseOut, onClick}) => {
  ...
  return (
    <Day
      ...
      {...((hasTournament && currentMonth) && {onMouseOver: onMouseOver})}
      onMouseOut={onMouseOut}
      {...((hasTournament && currentMonth) && {onClick:onClick})}
      hasTournament={hasTournament}
    >
      <p>{date}</p>
      {( hasTournament && currentMonth )? <img src={tournamentImage} style={{ width: '50%', height: '7vh' , borderRadius: '15px'}} /> : <div style={{height: '7vh'}}></div>}
      {(tournamentList.length > 1 && currentMonth )&& (
        <NotificationBubble>
