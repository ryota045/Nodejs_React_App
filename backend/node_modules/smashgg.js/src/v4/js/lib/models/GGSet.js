...
var PlayerLite_1 = require("./PlayerLite");
...
var GGSet = /** @class */ (function (_super) {
    ...
    GGSet.parse = function (data) {
        ...
        var p1 = PlayerLite_1.PlayerLite.parse(displayScoreParsed.tag1, data.slots[0]);
        var p2 = PlayerLite_1.PlayerLite.parse(displayScoreParsed.tag2, data.slots[1]);
    ...
    GGSet.filterOutDQs = function (sets) {
        ...
        var displayScores = sets.map(function (set) { return set.getDisplayScore(); });
        return displayScores.includes('DQ') ?
            sets.filter(function (set) { return set.getDisplayScore() !== 'DQ'; })
            :
                sets;
    };
    GGSet.filterOutByes = function (sets) {
        ...
        return displayScores.includes('BYE') ?
            sets.filter(function (set) { return set.getDisplayScore() !== 'BYE'; })
            :
                sets;
    };
    GGSet.filterOutResets = function (sets) {
        Logger_1.default.debug('GGSet.filterOutResets called');
        var fullRoundTexts = sets.map(function (set) { return set.getFullRoundText(); });
        return fullRoundTexts.includes('Grand Final Reset') ?
            sets.filter(function (set) { return set.getFullRoundText() !== 'Grand Final Reset'; })
            :
                sets;
    };
    ...
    GGSet.getDefaultSetOptions = function () {
        return {
            page: null,
            perPage: null,
            sortBy: null,
    ...
    // simple
    GGSet.prototype.getId = function () {
        return this.id;
    };
        ...
        if (this.winnerId && this.player2.entrantId && this.player1.entrantId)
            switch (this.winnerId) {
                ...
                case this.player2.entrantId:
                    return this.player2;
                default:
                    throw new Error("Winner ID " + this.winnerId + " does not match either player ID: \n\t\t\t\t\t[" + [this.player1.entrantId, this.player2.entrantId].join(',') + "]");
            }
        else
